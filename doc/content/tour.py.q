...
{
  engine['`/ expr'] = wrapper("code", classes = "encoded")
  engine['`// expr'] = wrapper("code", classes = "decoded")
  engine['// expr'] = wrapper(classes = "symname")
}
...
| `/[<>] | `//[♦] | //[diamond] | `/[/=] | `//[≠] | //[not equal]
| `/[\[|] | `//[⟦] | //[left double square bracket] | `/[|\]] | `//[⟧] | //[right double square bracket]
| `/[\{|] | `//[⦃] | //[left double curly bracket] | `/[|\}] | `//[⦄] | //[right double curly bracket]
| `/[<|] | `//[◀] | //[left triangle] | `/[|>] | `//[▶] | //[right triangle]
| `/[``] | `//[`] | //[backtick] | `/[\`br`] | `//[↲] | //[newline]

In a nutshell, suppose you have the following line of code:

ug % print["x >= 0 and x =< 10"]

When executed, it will print the following to the terminal:

% x ≥ 0 and x ≤ 10

As indicated in the table, `[>=] turned into `[≥] and `[=<] turned
into `[≤]. If you want to actually print out ">=" or "=<", you can
_escape one of the characters like this:
...
newlines, in strings, are encoded as `[`br`]. `[\n] _[will not work].
In fact, `[\\] escapes nothing at all in Unholy Grail (escape `["] by
doubling it).
...
Basic data structures in UG are __lists:
...
however define __[ordered dicts], which do preserve the order of keys:
...
but perhaps you lose some information in the process. UG's _[ad hoc]
structs can help:
...
In addition to all this, UG provides _immutable versions of each
...
assignment. In fact, its priority is _[dead last]. Always.
...
You can _type variables as you declare them.
...
The type checks are __dynamic: they are done at runtime and not at
...
more generally than in Python. Indent and line breaks obey two
_[very simple] rules:

# __Any indented block is equivalent to putting __[(] at the end of
  the line right before it, and __[)] at the end of its last line.
     * __Unless it is within any type of brackets. Inside `(),
       `[[]] and `{} all indent is _ignored.

# There is an implicit __comma (`[,]) at the end of __every line,
  unless it is continuated on the next line, or if the comma would
  come right after an opening bracket or right before a closing
  bracket.
...
because `if/else must be comma-separated in a `(...) block. The third
way will work everywhere.
...
`map returns a _generator: it does not do anything until it is asked
...
The right hand side of `each or `map can be a function, but there is a
caveat: most functions with a single argument take a _tuple of length
one. To illustrate:
...
iteration. `continue will skip to the next element,
_[yielding nothing] if it is in `map.
...
However, there is a shorter way to write this using _guards:
...
next to each other. Usually, you want to give the function a _list of
...
If there is an indented block after `[!], each line will be
interpreted as an element of a list:
...
It is possible to define _[multiple clauses], with one arrow per
...
`try/except). The `finally clause will always be executed at the end,
